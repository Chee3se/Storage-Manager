...
export default function SupplierSearch({ className = '' }) {
    const input = useRef()
    ...
    const fetchEntries = async () => {
        try {
            const response = await axios.get(route('admin.suppliers.index'), {
                params: {
                    search,
                    page,
                    perPage,
    ...
    return (
        <div className={`bg-stone-100 dark:bg-gray-950 px-4 py-2 rounded-md ${className}`}>
            <div className="relative flex items-center gap-2">
                ...
                <AddSupplierForm onClose={closeAddSupplierModal} show={showAddSupplierModal}/>
            </div>
